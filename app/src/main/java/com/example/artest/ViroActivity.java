...
public class ViroActivity extends Activity {
    ...
    private class Draggable3DObject {
        ...
        private void addModelToPosition(Vector position) {
            ...
            object3D.setScale(new Vector(.003f, .003f, .003f));
            ...
            object3D.loadModel(mViroView.getViroContext(), Uri.parse("file://"+mFileName), Object3D.Type.GLTF , new AsyncObject3DListener() {
                @Override
                ...
                public void onObject3DFailed(String s) {
                    Toast.makeText(ViroActivity.this, "An error occured when loading the 3D Object! :: "+s,
    ...
    private void displayScene() {
        ...
        spotlight.setPosition(mScene.getRootNode().getPositionRealtime());

        spotlight.setDirection(new Vector(5 , -5, 2));

        spotlight.setAttenuationStartDistance(5);
        spotlight.setAttenuationEndDistance(10);

        spotlight.setInnerAngle(0);
        spotlight.setOuterAngle(20);

        spotlight.setColor(Color.WHITE);
        spotlight.setIntensity(50f);

        mScene.getRootNode().addLight(spotlight);

        AmbientLight ambient = new AmbientLight(Color.WHITE, 20f);
        ...
        mScene.getRootNode().addChildNode(lightNode);
    ...
    public void showPopup(View v) {
        if(count == 0){
            placeObject(new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "UPES_MAP.gltf").toString());
        }
    }
